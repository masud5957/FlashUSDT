...
const configSchema = z.object({ walletAddress: z.string().regex(/^0x[a-fA-F0-9]{40}$/, 'Enter a valid BEP-20 wallet address'), qrCodeDataUrl: z.string().url('Enter a valid QR image URL').max(2048).nullable().optional(), instructions: z.string().max(2000).nullable().optional(), telegramUrl: z.string().url('Enter a valid Telegram URL').regex(/^https:\/\/t\.me\//, 'Telegram URL must start with https://t.me/').nullable().optional() })
...
export async function PUT(request: Request) {
  ...
  try {
    const db = getDb()
    const values = { network: 'BEP-20', walletAddress: parsed.data.walletAddress, qrCodeDataUrl: parsed.data.qrCodeDataUrl ?? null, instructions: parsed.data.instructions ?? null, telegramUrl: parsed.data.telegramUrl ?? null, updatedAt: new Date() }
