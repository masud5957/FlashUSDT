...
const tokenOptions = [
  { name: 'USDT', network: 'ERC-20', symbol: '◆', tone: 'text-slate-200', bg: 'bg-slate-700/60' },
  { name: 'USDT', network: 'TRC-20', symbol: '△', tone: 'text-red-400', bg: 'bg-red-500/15' },
  { name: 'USDT', network: 'BEP-20', symbol: '✥', tone: 'text-yellow-400', bg: 'bg-yellow-400/15' },
  { name: 'USDT', network: 'MATIC', symbol: '⬡', tone: 'text-violet-400', bg: 'bg-violet-500/15' },
]
const faqs = [
...
function TokenMark({ symbol, tone, bg, network }: { symbol: string; tone: string; bg: string; network: string }) {
  return <span aria-label={`${network} token symbol`} className={`mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-white/10 ${bg} ${tone} shadow-inner`}><span className="text-4xl font-black leading-none">{symbol}</span></span>
}
